...
public class AcronymOps implements ConfigurableOps,
	...
	public void onConfiguration(Activity activity, boolean firstTimeIn) {
		...
		if (firstTimeIn) {
			...
			// TODO - done - you fill in here, replacing "null" with the
			// appropriate initialization of the proxy.
			mAcronymWebServiceProxy = new RestAdapter.Builder()
					.setEndpoint(AcronymWebServiceProxy.ENDPOINT).build()
	...
	public List<AcronymExpansion> doInBackground(String acronym) {
		try {
			...
			// service.
				...
				// TODO -done- you fill in here, replacying "null" with a
				// call to the appropriate method on the proxy.
				final List<AcronymData> acrosData = mAcronymWebServiceProxy
						.getAcronymResults(acronym);

				if (acrosData.size() > 0) {
					AcronymData result = acrosData.get(0);

 ...
 */
public class VideoOps implements GenericAsyncTaskOps<Void, Void, List<Video>>,
	...
	/**
	 * VideoDataMediator mediates the communication between Video Service and
	...
	public void uploadVideo(Long videoId, Uri videoUri) {
		...
		contentValues.put(AcronymEntry.COLUMN_VIDEO_ID, videoId.toString());
		contentValues.put(AcronymEntry.COLUMN_TYPE, "0");
		contentValues.put(AcronymEntry.COLUMN_LOCATION, "0");
		contentValues.put(AcronymEntry.COLUMN_RAITING, 0);
		contentValues.put(AcronymEntry.COLUMN_TITLE, "0");

		mContext.getContentResolver().insert(AcronymEntry.CONTENT_URI,
				contentValues);
		...
		mVideoView
				...
				.getApplicationContext()
				.startService(
						UploadVideoService.makeIntent(mVideoView.get()
								.getApplicationContext(), videoId, videoUri));
		
		getVideoList();
	}

	/**
	 * Gets the VideoList from Server by executing the AsyncTask to expand the
	 * acronym without blocking the caller.
	 */
	 ...
	 * synchronous two-way method call, which runs in a background thread to
